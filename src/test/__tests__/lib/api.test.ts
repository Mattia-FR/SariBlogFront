...
describe('API Client', () => {
  ...
  it('devrait gérer les erreurs 401 en nettoyant le localStorage', () => {
    ...
    const responseInterceptor = mockAxios.create().interceptors.response.use.mock.calls[0][1];
  ...
  it('devrait passer les réponses réussies sans modification', () => {
    ...
    const responseInterceptor = mockAxios.create().interceptors.response.use.mock.calls[0][0];
