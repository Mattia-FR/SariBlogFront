...
import { mockUser, mockValidToken, mockApiResponse, mockApiError } from '../../utils/test-utils';
...
describe('useAuth', () => {
  ...
  it('devrait se connecter avec succès', async () => {
    ...
    expect(result.current.user).toEqual(mockUser);
    expect(result.current.token).toBe(mockValidToken);
    expect(localStorage.setItem).toHaveBeenCalledWith('admin_token', mockValidToken);
  ...
  it('devrait gérer les erreurs de connexion', async () => {
    ...
    mockApi.post.mockRejectedValue(mockApiError(401, 'Email ou mot de passe incorrect'));
    ...
    await act(async () => {
      ...
      expect(loginResult.error?.message).toBe('Email ou mot de passe incorrect');
    });
