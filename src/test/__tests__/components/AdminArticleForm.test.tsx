...
import { mockTag, mockFormData } from '../../utils/test-utils';
...
describe('AdminArticleForm', () => {
  ...
  it('devrait afficher l\'état d\'upload en cours', async () => {
    const mockUploadSingle = vi.fn().mockImplementation(() => 
  ...
  it('devrait afficher le nom du fichier uploadé', () => {
    ...
    expect(screen.getByText('Image sélectionnée: uploaded-image.jpg')).toBeInTheDocument();
  });
  ...
  it('devrait afficher le bon texte pour le mode édition', () => {
    ...
    expect(screen.getByRole('button', { name: /modifier l'article/i })).toBeInTheDocument();
  });

  it('devrait afficher le bon texte pour le mode création', () => {
    render(<AdminArticleForm {...defaultProps} />);

    expect(screen.getByRole('button', { name: /créer l'article/i })).toBeInTheDocument();
  });
  ...
  it('devrait gérer les erreurs d\'upload', async () => {
    const mockUploadSingle = vi.fn().mockRejectedValue(new Error('Upload failed'));
    mockUseAdminUpload.mockReturnValue({
      uploadSingle: mockUploadSingle,
    });
    ...
    await waitFor(() => {
      expect(consoleSpy).toHaveBeenCalledWith('Erreur upload:', expect.any(Error));
    });
